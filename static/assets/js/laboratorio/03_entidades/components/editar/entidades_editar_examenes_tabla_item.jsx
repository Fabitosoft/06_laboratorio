...
export default class TablaItemExamenes extends Component {
    ...
    render() {
        ...
        const {actualizarExamenEntidad, traerEntidad, notificarAction} = this.props;
        let es_activo = null;
        if (activo) {
            es_activo = <i
                ...
                onClick={() => {
                    actualizarExamenEntidad({...examen, activo: false}, () => {
                        notificarAction(`Se ha inactivado el examen ${examen_nombre}`);
            ...
            es_activo = <i
                ...
                onClick={() => {
                    actualizarExamenEntidad({...examen, activo: true}, () => {
                        notificarAction(`Se ha activado el examen ${examen_nombre}`);
                        traerEntidad(entidad);
                    })
                }
                }
            ...
            valor =
                <input
                    ...
                    onBlur={(e) => {
                        ...
                        const valor_antes = examen.valor_examen;
                        const examen_entidad = {...examen, valor_examen: parseFloat(valor_examen_campo)};
                        actualizarExamenEntidad(examen_entidad, () => {
                            notificarAction(`Se ha cambiado el valor del examen ${examen_nombre} de ${formatMoney(Number(valor_antes), "$", 0, ".", ",")} a ${formatMoney(Number(valor_examen_campo), "$", 0, ".", ",")}`,null,7000);
                            traerEntidad(entidad)
                        })
                    }}
                    type="text"
